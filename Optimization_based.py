...
def detect_price_change_points(day_data):
    price_values = day_data[['ElectricityPrice']].values
...
def detect_pv_change_points(day_data):
    ...
    total_day_pv = day_data['PV'].sum()
    return time_changes_pv, total_day_pv

def generate_smart_time_slots(df_hourly):
    """Returns {day: [(HH:MM, HH:MM) * 6]} exactly as your original code."""
    time_slots_per_day = {}
    for day, day_data in df_hourly.groupby(df_hourly['DateTime'].dt.date):
...
def assign_data_to_time_slots_single(df_hourly, time_slots_per_day):
    """Same as yours, but only ElectricityPrice, Load, PV."""
    out = []
    for day, slots in time_slots_per_day.items():
        day_data = df_hourly[df_hourly['DateTime'].dt.date == day]
...
def mpc_opt_single(df_slots, SOC0, SOC_min, SOC_max, Pbat_chargemax=9, Qbat=75):
    df = df_slots.copy()
    ...
    f = np.hstack([np.zeros(N), df['ElectricityPrice'].values])
